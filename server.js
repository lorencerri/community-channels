...
const Discord = require('discord.js'),
      ...
      dbl = new DBL('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjM4NjA1OTc0MDgwNTA3MDg0OCIsImJvdCI6dHJ1ZSwiaWF0IjoxNTIyMTIwMzY0fQ.-DuQZ8GixTXFc4KWP9B88PkrkUc4VZFgg3etAaMST_g', client),
...
client.on('ready', () => {
  setInterval(async function() {
	...
	let fetched = await db.fetch('timers', { target: timer });
    if (!fetched || !fetched instanceof Array) return;
    db.delete('timers', { target: timer });
...
client.on('message', async message => {
    ...
    // Return Statement
    if (message.content.toLowerCase().trim() !== fetched.msg && fetched.msg !== null) return message.delete({timeout: 1000});
...
client.on('message', async message => {
  ...
  if (delList !== null && delList.includes(message.channel.id) && !message.author.bot) message.delete({timeout: 1000})
  ...
  try { // Initialize
    ...
    if (completed) {
      ...
      db.add(`commands`, 1, {target: 'total'});
      db.add(`commands`, 1, {target: `${cmd}`})
    }
...
client.on('guildMemberAdd', async member => {
    ...
    active.forEach(async function(invite){
        ...
        db.set(`invites_${member.guild.id}`, invite.uses, { target: `.${invite.channel.id}.uses` })
    })
    
})

// Login
client.login("Mzg2MDU5NzQwODA1MDcwODQ4.Db0CHQ.NATG_iknotHM61QDH5hdWZPAgsc");
