  ...
  updateChannels: async function(guild) {
    ...
    if (guild.id === '435093693402316800') {
      ...
      embed.setColor(message.embeds[0].color)
        ...
        .setFooter('Community Channels Developed By Plexi Development @ discord.gg/plexidev | ~info for more information.')
      ...
      db.fetchArray(`officialChannels_${message.guild.id}`).then(o => {
        ...
        for (var i = 0; i < o.length; i++) {
          if (!message.guild.channels.get(o[i])) console.log('Fetched channel not found.')
            ...
            joinList.push(o[i])
            officialChannels += `**${i+1}** [❱](http://) ${message.guild.channels.get(o[i]).name.charAt(0).toUpperCase() + message.guild.channels.get(o[i]).name.slice(1).replace('nsfw', 'NSFW')}`
        ...
        db.fetchArray(`publicChannels_${message.guild.id}`).then(p => {
          ...
          for (var i = 0; i < p.length; i++) {
            if (!message.guild.channels.get(o[i])) console.log('Fetched channel not found.')
              ...
              joinList.push(p[i])
              publicChannels += `**${i+1+index}** [❱](http://) ${message.guild.channels.get(p[i]).name.charAt(0).toUpperCase() + message.guild.channels.get(p[i]).name.slice(1).replace('nsfw', 'NSFW')}`
