...
exports.run = async (client, message, args, tools) => {
    ...
    for (var i in list) {
        count++;
        array.push({ name: i, count: list[i] })
    }
    
    arraySort(array, 'count', {reverse: true});
    
    for (var i in array) {
        final.push([array[i].name, array[i].count]);
    }
    
    final = table.table(final, { border: { topBody: `─`, topJoin: `┬`, topLeft: `┌`, topRight: `┐`, bottomBody: `─`, bottomJoin: `┴`, bottomLeft: `└`, bottomRight: `┘`, bodyLeft: `│`, bodyRight: `│`, bodyJoin: `│`, joinBody: `─`, joinLeft: `├`, joinRight: `┤`, joinJoin: `┼` } })
