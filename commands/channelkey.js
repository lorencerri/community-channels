...
exports.run = async (client, message, args, tools) => {
    ...
    switch (pool.includes(message.channel.id)) {
        case true:
            ...
            await db.set(`customChannels_${message.guild.id}`, pool, { target: '.pool' });
            break;
        case false:
            ...
            db.push(`customChannels_${message.guild.id}`, message.channel.id, { target: '.pool' });
            db.set(`customChannels_${message.guild.id}`, { code: args.join(' ').toLowerCase() }, { target: `.${message.channel.id}` });
            break;
    }
